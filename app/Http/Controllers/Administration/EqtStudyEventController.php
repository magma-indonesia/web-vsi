...
class EqtStudyEventController extends Controller
{
    ...
    public function get(Request $request)
    {
        try {
            ...
            $data = EqtStudyEvent::when($name, function ($query) use ($name) {
    ...
    public function edit(Request $request)
    {
        ...
        $retrieve = EqtStudyEvent::where("id", $id)->first();
        if (!$retrieve) {
            abort(404);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $title = EqtStudyEvent::where('route', strtolower(preg_replace('/\s+/', '-', preg_replace('/[^a-zA-Z0-9_ -]/s','',$request->title))))
                        ->get();
            if (count($title) > 0) {
    ...
    {
        try {
            $data = EqtStudyEvent::where("id", $request->id)->first();
            if (!$data) {
                return response()->json([
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $n = EqtStudyEvent::where('id', $request->id)->first();
            if (!$n) {
                DB::commit();
                return response()->json([
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            $vb = EqtStudyEvent::where('id', $request->id)->first();
            if (!$vb) {
                DB::commit();
                return response()->json([
                    'message' => "Gagal mendapatkan data.",
