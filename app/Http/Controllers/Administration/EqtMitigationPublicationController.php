...
class EqtMitigationPublicationController extends Controller
{
    ...
    public function get(Request $request)
    {
        try {
            ...
            $data = EqtMitigationPublication::when($name, function ($query) use ($name) {
    ...
    public function edit(Request $request)
    {
        ...
        $retrieve = EqtMitigationPublication::where("id", $id)->first();
        if (!$retrieve) {
            abort(404);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $n = new EqtMitigationPublication();
            $n->content = $request->desc;
            $n->thumbnail = $request->thumbnail;
            $n->created_at = $request->created_at;
            $n->created_by = Auth::user()->name;

            $title = EqtMitigationPublication::where('route', strtolower(preg_replace('/\s+/', '-', preg_replace('/[^a-zA-Z0-9_ -]/s','',$request->title))))
                        ->get();
            if (count($title) > 0) {
    ...
    {
        try {
            $data = EqtMitigationPublication::where("id", $request->id)->first();
            if (!$data) {
                return response()->json([
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $n = EqtMitigationPublication::where('id', $request->id)->first();
            if (!$n) {
                DB::commit();
                return response()->json([
    ...
    public function delete(Request $request)
    {
        DB::beginTransaction();
        try {
            $vb = EqtMitigationPublication::where('id', $request->id)->first();
            if (!$vb) {
                DB::commit();
                return response()->json([
                    'message' => "Gagal mendapatkan data.",
