        ...
        <div class="form-group" id="roles_form_group">
            @foreach($allRoles as $key => $role)
                <div class="form-check">
                    <input class="form-check-input @if($errors->has('roles')) is-invalid @endif" type="checkbox"
                           name="roles[]" value="{{ $role->id }}" id="role_checkbox_{{$key}}"
                           @cannot(Itstructure\LaRbac\Models\Permission::ASSIGN_ROLE_FLAG, Itstructure\LaRbac\Classes\MemberToRole::make($user, $role)) disabled
                           @endcannot
